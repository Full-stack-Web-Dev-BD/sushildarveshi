...
module.exports = {
    createTransection(req, res) {
        new Transection(req.body)
            .save()
            .then(doc => {
                return res.status(200).json({ message: "Crated !" })
            })
            .catch(err => {
                console.log(err);
    ...
    update(req, res) {
        console.log('called')
        Transection.findOneAndUpdate({ _id: req.body.id }, req.body)
            .then(doc => {
                return res.status(200).json(doc)
            })
            .catch(err => {
    ...
    delete(req, res) {
        Transection.findByIdAndDelete({ _id: req.body.id })
            .then(doc => {
                return res.status(200).json(doc)
            })
            .catch(err => {
    ...
    filterTransectionByMonth(req, res) {
        ...
        async function collectData() {
            let filterData = allMonth.map(async el => {
                await Transection.find({ date: { $gt: new Date(el.gt), $lt: new Date(el.lt) } })
                    .then(rl => {
